<?php 
    ...
    if  (isset($_GET['Id_Salida'])) {
        $Id = $_GET['Id_Salida'];
        $Sentencia = $conexion->prepare('select * from salidas where Id_Salida = ?;');
        $Sentencia ->execute([$Id]); 
        $datos =  $Sentencia ->fetch(PDO::FETCH_ASSOC);
        if ($Sentencia-> rowCount() > 0) {
            $Nombre = $datos['Id_Salida'];
      ...
      if (isset($_POST['Enviar'])) { 
		...
		$Hora= $_POST['Hora']; 
        $Fecha = $_POST['Fecha'];
		$PreecioTotal= $_POST['PrecioTotal']; 
        $Sentencia = $conexion->prepare('update salidas set  Nombre = ?, Hora = ?, Fecha = ?, PrecioTotal = ? where Id_Salida = ?');
        $arrParams=array($Nombre,$Hora, $Fecha, $PrecioTotal,$Id);
						...
						<div class="info">
							<a data-toggle="collapse" href="#collapseExample" aria-expanded="true">
								<span>
									...
									<span class="user-level"><?php echo $datos1['Nombre']." ".$datos1['Apellido'];?></span>
                    ...
                    <div class="row">
						<div class="col-md-12">
							<div class="card">
								...
								<div class="card-body">
									<form action="EditarSalida.php?Id_Salida=<?php echo $_GET['Id_Salida']; ?>" method="post">
												<div class="row">
													...
													<div class="col-md-6 col-lg-6">
														<div class="form-group">
															...
															<input type="time" class="form-control" name="Hora" value="<?php echo $datos['Hora'] ?>">
														</div>
													</div>			
												</div>
												<div class="row">
													...
													<div class="col-md-6 col-lg-6">
														<div class="form-group">
															<label for="email2">PrecioTotal</label>
															<input type="number" class="form-control" name="PrecioTotal" value="<?php echo $datos['PrecioTotal'] ?>">
														</div>
													</div>			
												</div>
										</div>
										<div class="card-action">
											<button name="Enviar" class="btn btn-success">Editar Salida</button>
